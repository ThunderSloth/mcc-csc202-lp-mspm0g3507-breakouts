# ---------------------------------------
# Default behavior:
#   make / make all         -> builds BOTH boards (2-term + 3-term) + syncs docs + postprocess docs
...
list:
	...
	@for b in $(BOARDS); do \
	  pd=$$(ls -1 hardware/pcb/kicad/$$b | head -n 1); \
	  echo ""; \
...
build_one: check
	@PROJECT_DIR=$$(ls -1 hardware/pcb/kicad/$(BOARD) | head -n 1); \
	PROJECT="$$PROJECT_DIR"; \
...
check:
	...
	@test -d "hardware/pcb/kicad/2-term" || (echo "Missing board dir: hardware/pcb/kicad/2-term" && exit 1)
	@test -d "hardware/pcb/kicad/3-term" || (echo "Missing board dir: hardware/pcb/kicad/3-term" && exit 1)
